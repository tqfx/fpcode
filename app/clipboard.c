...
{
    /**
     * OpenClipboard 打开剪切板：指定关联到打开的剪切板的窗口句柄，传入NULL表示关联到当前任务。每次只允许一
    ...
    */
    ...
    {
        size_t n =
            (strlen(str) + 1) * sizeof(char); /**根据环境变量获取数据长度*/
        /**
        ...
        */
        ...
        if (hmem == NULL)
        {
            return CloseClipboard();  ///< 关闭剪切板,释放剪切板所有权，关闭后就不能写入数据
        }
        ...
        */
        ...
        UINT format =
            (sizeof(TCHAR) == sizeof(WCHAR)) ? CF_UNICODETEXT : CF_TEXT;
