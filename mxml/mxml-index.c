  ...
  */
    ...
    */
        ...
        while ((last - first) > 1)
        {
            ...
            if ((diff = index_find(ind, element, value, ind->nodes[current])) ==
                0)
            {
                /*
    ...
    const char * attr)     /* I - Attribute to index or @code NULL@ for none */
{
  ...
  */
    ...
    if (!element && !attr)
        ...
        current =
            mxmlFindElement(node, node, element, attr, NULL, MXML_DESCEND);
	...
	*/
                ...
                mxml_error(
                    ...
                    (int)((ind->alloc_nodes + 64) * sizeof(mxml_node_t *)),
                    strerror(errno));
        ...
        current =
            mxmlFindElement(current, node, element, attr, NULL, MXML_DESCEND);
    }
...
#ifdef DEBUG
    {
        ...
        if (attr)
        {
            ...
            for (i = 0; i < ind->num_nodes; i++)
                printf("%8d  %-8p  %-14.14s  %s\n", i, ind->nodes[i], ind->nodes[i]->value.element.name, mxmlElementGetAttr(ind->nodes[i], attr));
        }
        else
        {
