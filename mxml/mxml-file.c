           ...
           mxml_load_cb_t cb)  /* I - Callback function or constant */
{
  ...
  */
    ...
    return (
        mxml_load_data(top, &buf, cb, mxml_fd_getc, MXML_NO_CALLBACK, NULL));
}
             ...
             mxml_load_cb_t cb)  /* I - Callback function or constant */
{
  ...
  */
    ...
    return (
        mxml_load_data(top, fp, cb, mxml_file_getc, MXML_NO_CALLBACK, NULL));
}
                ...
                int *line) /* IO - Current line number */
{
    ...
    *entptr = '\0';
    ...
    if (ch != ';')
    {
        mxml_error("Character entity '%s' not terminated under parent <%s> on "
                   "line %d.",
                   entity,
                   parent ? parent->value.element.name : "null",
                   *line);
    ...
    if (mxml_bad_char(ch))
    {
        mxml_error("Bad control character 0x%02x under parent <%s> on line %d "
                   "not allowed by XML standard.",
                   ch,
                   parent ? parent->value.element.name : "null",
                   *line);
        return (EOF);
  ...
  */
	...
	*/
	  ...
	  */
                    ...
                    mxml_error("Unable to add processing instruction node to "
                               "parent <%s> on line %d.",
                               parent ? parent->value.element.name : "null",
                               line);
                    goto error;
                ...
                if (sax_cb)
                {
                    ...
                    if (strncmp(node->value.element.name, "?xml ", 5) &&
                        !mxmlRelease(node))
                        node = NULL;
                }
	...
	*/
	  ...
	  */
                    ...
                    mxml_error("Early EOF in declaration node on line %d.",
                               line);
                    goto error;
                }
	...
	*/
	  ...
	  */
                    ...
                    mxml_error("Mismatched close tag <%s> under parent <%s> on "
                               "line %d.",
                               buffer,
                               parent ? parent->value.element.name : "(null)",
                               line);
	...
	*/
	  ...
	  */
                ...
                else if (ch == '/')
                {
                    ...
                    {
                        mxml_error("Expected > but got '%c' instead for "
                                   "element <%s/> on line %d.",
                                   ch,
                                   buffer,
                                   line);
      ...
      */
            ...
            if ((ch = mxml_get_entity(parent, p, &encoding, getc_cb, &line)) ==
                EOF)
                goto error;
    ...
    if (parent)
    {
        ...
        if (node != parent)
        {
            mxml_error("Missing close tag </%s> under parent <%s> on line %d.",
                       ...
                       node->parent ? node->parent->value.element.name
                                    : "(null)",
                       line);
  ...
  */
    ...
    */
      ...
      */
            ...
            if (quote != '>')
            {
                mxml_error("Expected '>' after '%c' for element %s, but got "
                           "'%c' on line %d.",
                           ch,
                           node->value.element.name,
                           quote,
    ...
    */
      ...
      */
        ...
        *ptr = '\0';
        ...
        {
            mxml_error("Duplicate attribute '%s' in element %s on line %d.",
                       name,
                       node->value.element.name,
                       *line);
            goto error;
      ...
      */
            ...
            if (ch == EOF)
            {
                mxml_error("Missing value for attribute '%s' in element %s on "
                           "line %d.",
                           name,
                           node->value.element.name,
                           *line);
    ...
    */
        ...
        case ENCODE_UTF8:
            if (!(ch & 0x80))
            {
                ...
                if (mxml_bad_char(ch))
                {
                    mxml_error("Bad control character 0x%02x not allowed by "
                               "XML standard!",
                               ch);
                    return (EOF);
                }
	    ...
	    */
                ...
                if (ch < 0x80)
                {
                    mxml_error("Invalid UTF-8 sequence for character 0x%04x!",
                               ch);
                    return (EOF);
                }
  ...
  */
    ...
    */
	    ...
	    */
            ...
            else if (mxml_write_name(current->value.element.name, p, putc_cb) <
                     0)
                return (-1);
